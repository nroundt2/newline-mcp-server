...
// Direct API test script for Newline (CommonJS version)
...
async function getAuthToken() {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Authentication failed! Status: ${response.status}, Body: ${errorText}`);
  }
...
async function getSyntheticAccounts(authToken) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(`Failed to fetch accounts! Status: ${response.status}, Body: ${errorText}`);
  }
